...
binaryExpressionType :: BinaryOperator -> Type -> Type -> Java Type  -- left-hand and right-hand types
binaryExpressionType (LessThan) lhs_ty rhs_ty = booleanExpressionType lhs_ty rhs_ty
binaryExpressionType (LessThanEquals) lhs_ty rhs_ty = booleanExpressionType lhs_ty rhs_ty
binaryExpressionType (GreaterThan) lhs_ty rhs_ty = booleanExpressionType lhs_ty rhs_ty
binaryExpressionType (GreaterThanEquals) lhs_ty rhs_ty = booleanExpressionType lhs_ty rhs_ty
binaryExpressionType (Equality) lhs_ty rhs_ty = equalityExpressionType lhs_ty rhs_ty
binaryExpressionType (Disequality) lhs_ty rhs_ty = equalityExpressionType lhs_ty rhs_ty
binaryExpressionType (LogicalAnd) lhs_ty rhs_ty = logicalExpressionType lhs_ty rhs_ty
binaryExpressionType (LogicalOr) lhs_ty rhs_ty = logicalExpressionType lhs_ty rhs_ty
...
booleanExpressionType :: Type -> Type -> Java Type
booleanExpressionType lhs_ty rhs_ty = do
  case isConvertibleToNumericType lhs_ty of
    True -> do
      case isConvertibleToNumericType rhs_ty of
        True -> pure (PrimType Boolean)
        False -> issueError (ppr rhs_ty ++ " is not convertible to a numeric type.")
    False -> issueError (ppr lhs_ty ++ " is not convertible to a numeric type.")

logicalExpressionType :: Type -> Type -> Java Type
logicalExpressionType (PrimType Boolean) (PrimType Boolean) = pure (PrimType Boolean)
logicalExpressionType x y = issueError ("Illegal operand type(s) for conditional operation: " ++ show x ++ " and "++ show y)

equalityExpressionType :: Type -> Type -> Java Type
equalityExpressionType (PrimType NullType) (RefType _) = pure (PrimType Boolean)
equalityExpressionType (RefType _) (PrimType NullType) = pure (PrimType Boolean)
equalityExpressionType (PrimType NullType) (PrimType NullType) = pure (PrimType Boolean)
equalityExpressionType (RefType _) (RefType _) = pure (PrimType Boolean)
equalityExpressionType (PrimType Boolean) (PrimType Boolean) = pure (PrimType Boolean)
equalityExpressionType lhs_ty rhs_ty = do
  case (isConvertibleToNumericType lhs_ty && isConvertibleToNumericType rhs_ty ) of
    True -> pure (PrimType Boolean)
    False -> do
      case ((unboxConversionType_maybe lhs_ty),(unboxConversionType_maybe rhs_ty)) of
        (Nothing,Nothing) -> issueError ("Illegal operand type(s) for equality: <nulltype> and <nulltype>." )
        (Just lhs_type, Nothing) -> issueError ("Illegal operand type(s) for equality: " ++ show lhs_type ++ "and <nulltype>." )
        (Nothing, Just rhs_type) -> issueError ("Illegal operand type(s) for equality: <nulltype> and " ++ show rhs_type)
        (Just lhs_type, Just rhs_type) -> issueError ("Illegal operand type(s) for equality: " ++ show lhs_type ++ " and "++ show rhs_type)
