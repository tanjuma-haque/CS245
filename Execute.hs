...
trueOrFalse (BooleanV True) = True
trueOrFalse (BooleanV False) = False
trueOrFalse _ = error "huh?"
...
evalBinary op lhs_expr rhs_expr = do
 ...
 case op == LogicalAnd of
    True -> do
      case trueOrFalse lhs_val of
        True -> do
          rhs_res <- evalExpression lhs_expr
          rhs_val <- getValue rhs_res
          val <- performBinaryOp op lhs_val rhs_val
          pure (ValueR val)
        False -> do
          pure (ValueR (BooleanV False))
    False -> do
      case op == LogicalOr of
        True -> do
          case trueOrFalse lhs_val of
            True -> do
              pure (ValueR (BooleanV True))
            False -> do
...
performBinaryOp LogicalAnd lhs rhs = do
  case (lhs, rhs) of 
    (BooleanV True, BooleanV True ) -> pure (BooleanV True)
    (BooleanV True, BooleanV False ) -> pure (BooleanV False)


performBinaryOp LogicalOr lhs rhs = do
  case (lhs, rhs) of 
    (BooleanV False, BooleanV True ) -> pure (BooleanV True)
    (BooleanV False, BooleanV False ) -> pure (BooleanV False)

performBinaryOp Equality  lhs rhs = do
  case (lhs, rhs) of
    (BooleanV True, BooleanV True ) -> pure (BooleanV True)
    (BooleanV True, BooleanV False ) -> pure (BooleanV False)
    (BooleanV False, BooleanV True ) -> pure (BooleanV False)
    (BooleanV False, BooleanV False ) -> pure (BooleanV False)
    (ReferenceV (Null), ReferenceV (Null)) -> pure (BooleanV True)
    (ReferenceV (Null), _ ) -> pure (BooleanV False)
    ( _ , ReferenceV (Null)) -> pure (BooleanV False)
    (ReferenceV (Pointer x), ReferenceV(Pointer y)) -> do
      case x == y of
       True -> pure(BooleanV True)
       False -> pure(BooleanV False)
    ( _ , _ ) -> performNumericalOp (==) (==) (==) (==) noCheck lhs rhs


performBinaryOp Disequality lhs rhs = do
   case (lhs, rhs) of
    (BooleanV True, BooleanV True ) -> pure (BooleanV False)
    (BooleanV True, BooleanV False ) -> pure (BooleanV True)
    (BooleanV False, BooleanV True ) -> pure (BooleanV True)
    (BooleanV False, BooleanV False ) -> pure (BooleanV False)
    (ReferenceV (Null), ReferenceV (Null)) -> pure (BooleanV False)
    (ReferenceV (Null), _ ) -> pure (BooleanV True)
    ( _ , ReferenceV (Null)) -> pure (BooleanV True)
    (ReferenceV (Pointer x), ReferenceV(Pointer y)) -> do
      case x /= y of
       True -> pure(BooleanV True)
       False -> pure(BooleanV False)
...
{-
performBinaryOp Equality lhs rhs = do
  case isBooleanType lhs of
    True -> do
      case isBooleanType rhs of
        True -> do

  performNumericalOp (>=) (>=) (>=) (>=) noCheck lhs rhs
-}

isBooleanType :: Value -> Bool
isBooleanType (BooleanV True) = True
isBooleanType (BooleanV False) = True
